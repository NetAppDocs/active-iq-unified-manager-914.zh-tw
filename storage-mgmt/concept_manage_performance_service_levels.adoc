...
+
工作負載的 IOPs 參數會根據工作負載行為動態產生，並以格式附加至 Beyond Extreme PSL 的名稱 `Beyond Extreme <number-(peak IOPS/TB)> <number(expected IOPS/TB)>`。例如，如果系統判斷工作負載的尖峰和預期 IOPs 分別為 `106345`和 `37929`，則為工作負載產生的 Beyond Extreme PSL 會命名為 `Beyond Extreme 106345 37929`。雖然系統建議使用這些 UL ，但當您將它們指派給工作負載時，這些 UL 會標示為 `User-defined`「 In Type 」（類型）。
...
如需將系統建議的 ISL 指派給工作負載的相關資訊，請參閱link:..//storage-mgmt/concept_assign_policies_on_workloads.html#assigning-system-recommended-psls-to-workloads["將系統建議的PSL指派給工作負載"]
...
|===
如需尖峰 IOPS 與預期 IOP 如何協助在 ONTAP 叢集上實現一致的差異化效能的相關資訊，請參閱下列知識庫文章：https://kb.netapp.com/Advice_and_Troubleshooting/Data_Infrastructure_Management/Active_IQ_Unified_Manager/What_is_Performance_Budgeting%3F["什麼是績效預算？"]
