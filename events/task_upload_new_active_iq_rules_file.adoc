...
+
rules 檔案位於： https://mysupport.netapp.com/api/content-service/staticcontents/content/public/tools/unifiedmanager/ca/secure_rules.zip[]



.步驟
. 在具有網路存取權的電腦上，瀏覽至 NetApp 支援網站並下載目前的規則 `.zip`檔。
+
...
. 在 * 上傳規則 * 對話方塊中，瀏覽並選取您下載的規則 `.zip`檔，然後按一下 * 上傳 * 。
+
此程序可能需要幾分鐘的時間。
...
如需詳細資訊，請參閱本知識庫 (KB) 文件： https://kb.netapp.com/Advice_and_Troubleshooting/Data_Infrastructure_Management/Active_IQ_Unified_Manager/How_to_update_AIQCASecure_rules_manually_in_Active_IQ_Unified_Manager["如何手動更新AIQCASecure規則Active IQ Unified Manager 、請至英文版"]。
