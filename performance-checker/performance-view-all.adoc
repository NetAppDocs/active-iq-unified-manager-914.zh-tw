...
您可以(`.pdf`使用 *Reports* 按鈕(`.csv`，將這些頁面中(`.xlsx`的資料匯出至以逗號分隔的值）檔案， Microsoft Excel 檔案或）文件，然後使用匯出的資料來建立報告。此外、您也可以自訂頁面、然後使用*排程報告*按鈕、排程定期建立及以電子郵件寄送報告。
...
如果您想要分析特定物件的延遲和處理量，請按一下「更多選項」按鈕，然後按一下image:icon_kabob.gif[""]「 * 分析工作負載 * 」，即可在「工作負載分析」頁面上檢視效能和容量圖表。您可以在System Manager上檢視詳細資料、前提是您擁有System Manager的有效認證資料。
...
** SSD
** SSD（FabricPool 簡稱「HDD」）。結合SSD與雲端層
...
如果要分析特定物件的延遲和處理量，請按一下「更多」圖示image:icon_kabob.gif[""]，然後按一下「 * 分析工作負載 * 」，即可在 * 工作負載分析 * 頁面上檢視效能和容量圖表。
...
* 相關物件：指派給QoS原則群組的工作負載數量。您可以單擊 QoS 策略組名稱旁邊的展開按鈕（image:../media/chevron_down.gif[""]），以查看有關策略組的更多詳細信息。
* 已分配容量：QoS原則群組中物件目前使用的空間量。
* 相關物件：指派給QoS原則群組的工作負載數量、並分隔成磁碟區和LUN。您可以按一下號碼、瀏覽至提供所選磁碟區或LUN詳細資料的頁面。


如需詳細資訊，請參閱中的主題link:..//performance-checker/concept_manage_performance_using_qos_policy_group_information.html["使用QoS原則群組資訊來管理效能"]。
