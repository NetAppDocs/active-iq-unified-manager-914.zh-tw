...
[NOTE]
====
您可以將輸入參數設為 `true`，以擷取系統定義的效能服務層級詳細資料 `system_defined`。從輸出中、取得您要套用至檔案共用區的Performance Service層級關鍵。
...
[NOTE]
====
在資源配置NFS共用時、您可以提供所有必要的值來建立匯出原則、或是提供匯出原則金鑰、然後重複使用現有的匯出原則。如果您想要重複使用儲存VM的匯出原則、則需要新增匯出原則金鑰。除非您知道金鑰，否則可以使用 API 擷取匯出原則金鑰 `/datacenter/protocols/nfs/export-policies`。若要建立新原則、您必須輸入下列範例所示的規則。對於輸入的規則、API會嘗試比對主機、儲存VM和規則、以搜尋現有的匯出原則。如果有現有的匯出原則、就會使用該原則。否則會建立新的匯出原則。
...
+
您可以看到的 POST 方法 `/storage-provider/file-shares`會在內部呼叫每個功能所需的所有 API ，並建立物件。例如，它會叫用 `/storage-provider/performance-service-levels/` API 來指派檔案共用上的效能服務層級。
