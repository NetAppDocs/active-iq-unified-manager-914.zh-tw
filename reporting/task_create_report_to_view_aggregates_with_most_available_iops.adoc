...
. 選取 * 顯示 / 隱藏 * 以顯示 `Available IOPS`欄並隱藏 `Cluster FQDN, Inactive Data Reporting,`和 `Threshold Policy`欄。
. 拖放 `Available IOPS`欄旁邊的和 `Free Capacity`欄 `Type`。
. 命名並儲存自訂檢視 `Available IOPS Per Aggr.`
. 選擇*報告*>*下載Excel *。
+
...
. 設定下列排序層級：
+
.. 將 * 排序依據 * （ IOPS ）， * 排序依據 * AS 和 * 排序 * `Largest to Smallest.` AS 指定 `Available IOPS`為 `Cell Values,`
.. 按一下*新增層級*。
.. 將 * 排序依據 * 指定為， * 排序依據 `Type` * AS 和 * 排序 `Cell Values,` * AS `Z to A.`
.. 按一下*新增層級*。
.. 將 * 排序依據 * 指定為 * 排序依據 * AS ， * `Largest to Smallest.` 排序依據 * `Cell Values,` 指定為 `Free Capacity (GB),`
.. 按一下「*確定*」。
...
====
. 在此案例中，請選取您已修改的 Excel 檔案 `performance-aggregates-<date>.xlsx.`
. 按一下「*開啟*」。
. 按一下*提交*。
+
